...
def _test_crop(img_shape, desired_side=256, rot=0,
               img_gen=imread_img):
    """
    ...
    proposed_img = NewCrop(img, center, max_side, 0, desired_side)
    if not (new_img == proposed_img).flatten().all():
...
def test_crop_tall_small_small_no_rot():
    _test_crop((200, 100))
